...
st.set_page_config(page_title="Fridge Raider Visualizer", layout="wide")
...
<body>
    <div class="container">
        <header>
            <h1>Fridge Raider Visualizer</h1>
            <p class="subtitle">Type your ingredients below to see what you can cook!</p>
            
            <div class="search-container">
                <input type="text" id="fridgeInput" class="search-input" placeholder="e.g. Chicken, Spinach, Rice...">
            </div>
        </header>
    ...
    <script>
        ...
        function displayRecipes(data) {
            if (data.length === 0) {
                grid.innerHTML = '<p style="text-align:center; col-span:3; width:100%;">No recipes match those ingredients!</p>';
                return;
            }
        ...
        // --- FILTER LOGIC ---
        function filterAll() {
            const searchTerm = fridgeInput.value.toLowerCase();
            ...
            const filtered = recipes.filter(item => {
                ...
                // We check if the search term exists in Title OR Ingredients
        ...
        filterBtns.forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                filterAll(); // Re-run filter with new category
            });
        });
    </script>
</body>
